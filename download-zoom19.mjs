...
async function start() {
  console.log('⚡ Turbo Downloader: Fetching Zoom 19 Tiles Only...\n');
  const queue = [];
  ...
  console.log(`Remaining Zoom 19 tiles to fetch: ${total}\n`);
  let completed = 0;
  ...
  async function worker() {
    while (queue.length > 0) {
      ...
      completed++;
      process.stdout.write(`\r⚡ [Zoom 19 HD] Progress: ${completed}/${total} tiles (${Math.round((completed / total) * 100)}%)`);
    }
  }
  ...
  console.log('\n\n Done! Native Zoom 19 tiles saved to disk.\n');
}
